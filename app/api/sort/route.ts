...
const SortItemSchema = z.object({
  ...
  club: z
    .object({
      ...
      provider: z.string().describe("The organisation/venue running it, if given, else empty string"),
    })
    .nullable()
...
Also separately: if the text describes a child doing a club or extracurricular activity as a standing weekly thing (e.g. "which she does every Monday from 5.40 to 6.20pm", "he goes swimming on Wednesdays after school", not a one-off outing), set "club" to {"name":"<club/activity name>","weekday":"<day it's on>","timeFrom":"HH:MM or empty string","timeTo":"HH:MM or empty string","provider":"<who runs it/venue, if given, else empty string>"} tied to whichever known children it's for, so it can be offered as a save to their standing clubs list. Otherwise null. A one-off event on a specific date is a "reminder", not a "club".
  ...
  try {
    ...
    const items: PendingItem[] = arr.map((p) => {
      ...
      return {
        ...
        club: p.club?.name
          ? { name: p.club.name, weekday: p.club.weekday, timeFrom: p.club.timeFrom || "", timeTo: p.club.timeTo || "", provider: p.club.provider || "" }
          : null,
      };
    });
